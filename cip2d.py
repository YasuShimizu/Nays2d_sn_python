...
def u_cal2(fn,gxn,gyn,u,v_up,un,gux,guy,nx,ny,dsi,dn,dt):
    ...
    for i in np.arange(1,nx):
        for j in np.arange(1,ny+1):
            ...
            if j==ny:
                gyo=(u[i,j]-u[i,j-1])/dy
                guy[i,j]=guy[i,j]-(gxo*(u[i,j]-u[i,j-1])+gyo*(v_up[i,j]-v_up[i,j-1]))*dt/dy
            elif j==1:
                gyo=(u[i,j+1]-u[i,j])/dy
                guy[i,j]=guy[i,j]-(gxo*(u[i,j+1]-u[i,j])+gyo*(v_up[i,j+1]-v_up[i,j]))*dt/dy
            else:
...
def v_cal2(fn,gxn,gyn,u_vp,v,vn,gvx,gvy,nx,ny,ds,dnj,dt): 
    ...
    for i in np.arange(1,nx):
        for j in np.arange(1,ny):
            ...
            if i==nx:
                gxo=(v[i,j]-v[i-1,j])/dx
                gvx[i,j]=gvx[i,j]-(gxo*(u_vp[i,j]-u_vp[i-1,j])+gyo*(v[i,j]-v[i-1,j]))*dt/dx
            elif i==1:
                gxo=(v[i+1,j]-v[i,j])/dx
                gvx[i,j]=gvx[i,j]-(gxo*(u_vp[i+1,j]-u_vp[i,j])+gyo*(v[i+1,j]-v[i,j]))*dt/dx
            else:
